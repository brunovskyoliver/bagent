...
patterns = (
    ...
    r"\b([1-9][0-9]*) (?:(?:repository-relative|forbidden) )?(?:assertions?|surfaces?|canaries?|routes?|states?|files?|cases?|keys?|categories|matches|kill points?|SIGKILLs?|integrity checks?|restarts?|conversions?|campaigns?|works?|links?(?: checked)?|sessions?|retirements?|reloads?|chats?|commands?|checks?)\b",
